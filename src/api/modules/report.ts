...
export async function uploadReportMedia(files: File[]) {
  const formData = new FormData()
  files.forEach((file) => formData.append('files', file))
  return request.post<{ urls: string[] }>('/report/upload', formData, {
    headers: {
...
export function refreshReportAi(id: string) {
  return request.post<ReportItem>(`/report/${id}/refresh-ai`)
}
