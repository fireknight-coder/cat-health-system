...
import { getReportById, approveMatchExisting, approveNewCat, rejectReport, refreshReportAi } from '@/api/modules/report'
...
const topCandidate = computed(() => {
  const first = (report.value as any)?.aiTopK?.[0]
  if (!first) return null
  const catInfo = typeof first.catId === 'object' ? first.catId : {}
  const catId = (catInfo as any)?._id || first.catId || ''
  return {
    catId: String(catId),
    name: (catInfo as any)?.name || first.name || '未命名猫咪',
    avatar: (catInfo as any)?.avatar || first.avatar || '',
    similarity: Number(first.similarity || 0)
  }
})
...
async function handleRefreshAi() {
  ...
  try {
    const res = await refreshReportAi(id.value)
    report.value = (res as { data: ReportItem }).data
  ...
  <div class="page">
    ...
    <template v-else-if="report">
      ...
      <div class="section ai-result">
        <div class="ai-header">
          <h3>🤖 AI 处理结果</h3>
          <el-button size="small" :loading="refreshLoading" @click="handleRefreshAi">随机刷新建议</el-button>
        </div>
        <div class="ai-tags">
          <el-tag :type="report.aiSuggestion === 'existing' ? 'warning' : 'success'">
            新老猫建议：{{ report.aiSuggestion === 'existing' ? '建议老猫' : '建议新猫' }}
          </el-tag>
          <el-tag type="info">建议置信度：{{ (((report.aiSuggestionConfidence ?? 0.55) * 100)).toFixed(0) }}%</el-tag>
        ...
        <!-- AI 相似猫 Top-K -->
        <div class="topk-section" v-if="topCandidate">
          <h4>🐱 AI 相似猫候选</h4>
          <div class="topk-single">
            <img v-if="topCandidate.avatar" :src="topCandidate.avatar" class="candidate-avatar" />
            <span class="candidate-name">{{ topCandidate.name }}</span>
            <span class="similarity">{{ (topCandidate.similarity * 100).toFixed(1) }}% 相似</span>
            <el-button type="primary" link @click="selectedCatId = topCandidate.catId">
              选择该猫
            </el-button>
          </div>
        </div>
        <p v-else class="no-candidate">本次未发现相似猫，建议新猫建档</p>
      </div>
          ...
          <div class="action-form">
            <el-form :model="newCatForm" inline>
              ...
              <el-form-item label="性别">
                <el-select v-model="newCatForm.gender" style="width: 120px">
            ...
            <p class="gender-preview">当前性别选择：{{ newCatForm.gender === 'male' ? '公猫' : newCatForm.gender === 'female' ? '母猫' : '未知' }}</p>
...
.candidate-avatar { width: 36px; height: 36px; border-radius: 50%; object-fit: cover; }
.candidate-name { font-weight: 600; color: #303133; }
.topk-single .similarity { color: #67c23a; }
...
.gender-preview { color: #606266; font-size: 13px; margin: 6px 0 10px; }
