        ...
        <div v-if="result" class="result-content">
          ...
          <div class="compare-grid">
            <div class="compare-card">
              <span class="matched-cat-label">你上传的图片</span>
              <img
                v-if="previewUrl"
                :src="previewUrl"
                :alt="selectedFile?.name || '上传图片'"
                class="matched-cat-image"
              />
              <strong class="matched-cat-id">待识别图片</strong>
              <span class="matched-cat-name">{{ selectedFile?.name }}</span>
            </div>

            <div v-if="result.matchedCatImageUrl" class="compare-card">
              <span class="matched-cat-label">匹配猫档案首图</span>
              <img
                :src="result.matchedCatImageUrl"
                :alt="result.matchedCatImageName || '匹配猫图片'"
                class="matched-cat-image"
              />
              <strong class="matched-cat-id">档案编号 {{ result.bestCatId }}</strong>
              <span class="matched-cat-name">{{ result.matchedCatImageName }}</span>
            </div>

            <div v-else class="compare-card compare-empty">
              <span class="matched-cat-label">匹配猫档案首图</span>
              <div class="compare-placeholder">未命中已有档案</div>
              <strong class="matched-cat-id">新猫</strong>
              <span class="matched-cat-name">当前没有可展示的匹配猫图片</span>
            </div>
          </div>
...
.matched-cat-image {
  ...
  object-fit: cover;
  border-radius: 16px;
  box-shadow: 0 10px 22px rgba(101, 75, 51, 0.12);
}
