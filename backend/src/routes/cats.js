...
router.post('/upload', authenticateToken, catUpload.array('files', 10), async (req, res) => {
  try {
    const files = req.files || []
    const urls = files.map((file) => `${req.protocol}://${req.get('host')}/uploads/cats/${file.filename}`)
    res.json({ success: true, data: { urls } })
  } catch (error) {
    res.status(500).json({ success: false, message: '猫照片上传失败' })
  }
})
